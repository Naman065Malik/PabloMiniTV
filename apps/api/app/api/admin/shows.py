...
def create_show(data: ShowCreate, db: Session = Depends(get_db), current_user: User = Depends(require_editor)):
    try:
...
def get_show(show_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_editor)):
...
def update_show(show_id: int, data: ShowUpdate, db: Session = Depends(get_db), current_user: User = Depends(require_editor)):
    try:
...
def delete_show(show_id: int, db: Session = Depends(get_db), current_user: User = Depends(require_editor)):
    try:
