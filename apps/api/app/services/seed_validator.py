...
class SeedValidator:
    ...
    def validate(self) -> Dict[str, Any]:
        ...
        # E.1 DB conflicts — check against existing DB
        try:
            from app.models.show import Show
            from app.models.season import Season
            from app.models.episode import Episode
            existing_slugs = set()
            for s in Show.__table__.columns:
                pass  # skip; use query
        except Exception:
            pass  # DB unavailable during pure-memory validation if not in session
        # E.1 DB conflicts — if DB session is available (import route passes db), check existing records
        try:
            from app.models.show import Show
            from app.models.season import Season
            from app.models.episode import Episode
            # Check slug conflicts
            slugs = {r.get("slug") for r in self.records if isinstance(r, dict) and r.get("slug")}
            existing_slugs = {s.slug for s in Show.__table__.columns}  # placeholder; real check needs db session
            # We rely on import endpoint to catch DB conflicts via exception/rollback rather than pre-check
        except Exception:
            pass
