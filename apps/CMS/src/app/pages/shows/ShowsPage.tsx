    ...
    <section className="space-y-6">
      ...
      {isLoading ? <Skeleton count={7} /> : error ? (
        ...
        <div className="table-card overflow-x-auto">
          <table className="content-table"><thead><tr><th>Show title</th><th>Section</th><th>Category</th><th>Status</th><th>Updated</th><th /></tr></thead>
            <tbody>{data.items.map(show => <tr key={show.id}><td><strong>{show.title}</strong><span className="cell-subtitle">{show.description || 'No synopsis yet'}</span></td><td>{show.section || '—'}</td><td>{show.category || '—'}</td><td><span className={`status status-${show.status}`}>{show.status}</span></td><td>{new Date(show.updated_at).toLocaleDateString()}</td><td><Button asChild size="sm" variant="outline"><Link to={`/admin/shows/${show.id}/edit`}>Edit</Link></Button></td></tr>)}</tbody>
          </table>
