...
export function EpisodeForm({ initialData, shows, seasons, selectedShowId, onShowChange, onSubmit, onCancel, isSubmitting = false, error }: EpisodeFormProps) {
  ...
  return <form className="editor-form" onSubmit={handleSubmit}>
    ...
    <fieldset><legend>Episode information</legend><div className="form-grid"><label>Show<Select value={selectedShowId ?? ''} disabled aria-label="Show"><option value="">Select show</option>{shows.map(show => <option key={show.id} value={show.id}>{show.title}</option>)}</Select></label><label>Season<Select name="seasonId" defaultValue={initialData?.season_id ?? ''} required disabled aria-label="Season"><option value="">Select season</option>{seasons.map(season => <option key={season.id} value={season.id}>{season.season_number === 0 ? 'Trailer (Season 0)' : `Season ${season.season_number}`}{season.title ? ` — ${season.title}` : ''}</option>)}</Select></label></div>
