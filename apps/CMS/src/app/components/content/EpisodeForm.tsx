...
import { ArtworkUpload, type ArtworkType } from './ArtworkUpload'
import type { Episode, EpisodeInput, Season } from '../../api/episodes'
import type { Artwork, Show } from '../../api/shows'
...
export function EpisodeForm({ initialData, shows, seasons, selectedShowId, onShowChange, onSubmit, onCancel, isSubmitting = false, error, artwork, onArtworkUpload }: EpisodeFormProps) {
  ...
  const isBlocked = !initialData?.duration_seconds || (!thumbnail && !artwork)
  return <form className="editor-form" onSubmit={handleSubmit}>
    ...
    <fieldset><legend>Episode information</legend><div className="form-grid"><label>Show<Select value={selectedShowId ?? ''} onChange={event => onShowChange(Number(event.target.value))} required aria-label="Show" disabled={!!initialData}><option value="">Select show</option>{shows.map(show => <option key={show.id} value={show.id}>{show.title}</option>)}</Select></label><label>Season<Select name="seasonId" defaultValue={initialData?.season_id ?? ''} required disabled={!!initialData} aria-label="Season"><option value="">Select season</option>{seasons.map(season => <option key={season.id} value={season.id}>{season.season_number === 0 ? 'Trailer (Season 0)' : `Season ${season.season_number}`}{season.title ? ` — ${season.title}` : ''}</option>)}</Select></label></div>
    ...
    <fieldset><legend>Thumbnail</legend>{onArtworkUpload ? <ArtworkUpload type="THUMBNAIL" existingArtwork={artwork} onUpload={file => onArtworkUpload('THUMBNAIL', file)} /> : <label className="artwork-slot"><strong>Episode thumbnail</strong><span>16:9 · ~640 × 360 · max 200KB</span><input type="file" accept="image/*" onChange={event => { const file = event.target.files?.[0]; setThumbnail(file && file.size <= 200 * 1024 ? file.name : '') }} /></label>}{isBlocked && <p className="publish-blocked">Publishing blocked: add a thumbnail and a duration before publishing this episode.</p>}</fieldset>
