...
export default function EpisodeCard({ episode }: { episode: (typeof episodes)[number] }) {
  return (
    <article className="flex-[0_0_300px] bg-white rounded-[18px] p-2.5 shadow-[0_5px_18px_#34206b14]">
      <div className={`h-[155px] rounded-xl relative grid place-items-center overflow-hidden ${episode.artwork === "park" ? "bg-gradient-to-br from-[#f8c75d] to-[#64d49c]" : episode.artwork === "jungle" ? "bg-gradient-to-br from-[#45b886] to-[#714cbd]" : "bg-gradient-to-br from-[#41c7ed] to-[#1869c4]"}`}>
        <span className="w-[42px] h-[42px] rounded-full bg-white text-[#5230b2] grid place-items-center font-black">▶</span>
        <span className="absolute right-[10px] bottom-[8px] px-1.5 py-[3px] rounded-[6px] bg-[#261557cc] text-white text-xs font-extrabold">{episode.duration}</span>
      </div>
